...
begin
  try
    f:=TFilestream.Create(GameFuqRDir+'accesscheck.tmp', fmCreate);
    try
  ...
  try
    f:=TFilestream.Create(GameFuqRDir+'accesscheck.tmp', fmOpenReadWrite);
    try
      f.Seek(0,soFromEnd);
  ...
  if not deletefile(GameFuqRDir+'accesscheck.tmp') then
