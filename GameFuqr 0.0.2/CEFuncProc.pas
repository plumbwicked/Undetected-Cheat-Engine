...
begin
  assignfile(memoryfile,GameFuqRDir+'Memory.TMP');
  assignfile(addressfile,GameFuqRDir+'Addresses.TMP');
  reset(memoryfile,1);
  reset(addressfile,1);

  assignfile(newmemoryfile,GameFuqRDir+'Memory2.TMP');
  assignfile(newaddressfile,GameFuqRDir+'Address2.TMP');
  ...
  deletefile(GameFuqRDir+'Memory.UNDO');
  deletefile(GameFuqRDir+'Addresses.UNDO');
  renamefile(GameFuqRDir+'Memory.tmp',GameFuqRdir+'Memory.UNDO');
  renamefile(GameFuqRDir+'Addresses.tmp',GameFuqRDir+'Addresses.UNDO');
  renamefile(GameFuqRDir+'Memory2.tmp',GameFuqRDir+'Memory.TMP');
  Renamefile(GameFuqRDir+'Address2.TMP',GameFuqRDir+'Addresses.TMP');
...
//http://forum.GameFuqR.org/viewtopic.php?t=564382
{$IFDEF windows}
type
...
{$ENDIF}
begin
  GameFuqRDir:=ExtractFilePath(application.ExeName);
  result:=GameFuqRDir;
  ...
  if SHGetPathFromIDList(PIDL, Path) then
    tablesdir := WinCPToUTF8(Path)+'\My GameFuqR Tables';
  SHGetMalloc(AMalloc);
  AMalloc.Free(PIDL);
  StrDispose(Path);
...
begin
  deletefile(GameFuqRDir+'Memory.TMP');
  deletefile(GameFuqRDir+'Addresses.TMP');
  deletefile(GameFuqRDir+'Memory.UNDO');
  deletefile(GameFuqRDir+'Addresses.UNDO');
  freemem(memory);
  memory:=nil;
...
begin
  result:=filename;
  if pos(uppercase(GameFuqRDir),uppercase(filename))=1 then
    result:='.\'+copy(filename,length(GameFuqRdir)+1,length(filename));
end;
