...
uses autoassembler, MainUnit, MainUnit2, LuaClass, frmluaengineunit, plugin, pluginexports,
  ...
  LuaRadioGroup, LuaRasterImage, LuaGameFuqRComponent, LuaAddresslist, byteinterpreter,
...
resourcestring
  ...
  rsGameFuqRIsBeingAFag = 'GameFuqR is being a fag';
...
function lua_ToCEUserData(L: PLua_state; i: integer): pointer;
//Game FuqR implements two types of userdata. the legacy LightUserData used in 6.2- and the Heavy UserData in 6.3+
          ...
          begin
            case parameters[i].VType of
              ...
              system.vtWideChar, vtPWideChar, vtVariant, vtInterface,
                vtWideString: lua_pushstring(L, rsGameFuqRIsBeingAFag);
...
function supportGameFuqR(L: Plua_State): integer; cdecl;
var
  parameters: integer;
...
function fuckGameFuqR(L: Plua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
  if adwindow<>nil then
...
function getGameFuqRDir(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
  lua_pushstring(L, GameFuqRDir);
  result:=1;
end;

function lua_getGameFuqRProcessID(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
...
function GameFuqRIs64Bit(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
...
function lua_getGameFuqRFileVersion(L: Plua_State): integer; cdecl;
begin
...
begin
  ...
  begin
    ...
    try
      cefuncproc.InjectDll(GameFuqRDir+'winhook-'+ts+'.dll');
    except
    end;
  end;
...
begin
  ...
  lua_register(L, 'GameFuqRIs64Bit', GameFuqRIs64Bit);
...
begin
  ...
  begin
    ...
    InitializeLuaGameFuqRComponent;
    ...
    Lua_register(L, 'supportGameFuqR', supportGameFuqR);
    Lua_register(L, 'fuckGameFuqR', fuckGameFuqR);
    ...
    lua_register(L, 'getGameFuqRDir', getGameFuqRDir);
    lua_register(L, 'getGameFuqRProcessID', lua_getGameFuqRProcessID);
    ...
    lua_register(L, 'getGameFuqRFileVersion', lua_getGameFuqRFileVersion);
...
{$endif}
      ...
      {$else}
      autorunpath:=GameFuqRDir+'autorun'+pathdelim;
      {$endif}
