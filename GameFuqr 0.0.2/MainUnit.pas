...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure CloseGameFuqR1Click(Sender: TObject);
...
uses cefuncproc, MainUnit2, ProcessWindowUnit, MemoryBrowserFormUnit, TypePopup, HotKeys,
  ...
  ,frmFoundlistPreferencesUnit, fontSaveLoadRegistry{$ifdef windows}, fuQ'recoins{$endif},strutils;
...
resourcestring
  ...
  rsThankYouForTryingOutGameFuqRBecauseItHasExpired =
  ...
  rsLuaScriptGameFuqRTable = 'Lua script: GameFuqR Table';
  ...
  rsHex = 'Hex';
  rsDoYouWantToGoToTheGameFuqRWebsite =
  ...
  strEnableGameFuqR = 'Enable GameFuqR';
  strDisableGameFuqR = 'Disable GameFuqR';
  ...
  rsBringsGameFuqRToFront = 'brings Game FuqR to front';
...
begin
  {$ifdef windows}
  if aprilfools then decreaseGameFuqRECoinCount;
  {$endif}
...
//  symhandler.waitforsymbolsloaded;
  ...
  begin
    ...
    else if fileexists(GameFuqRdir + expectedfilename) then
      path:=GameFuqRdir + expectedfilename
...
begin
  ...
  p.Execute;
  {$else}
  shellexecute(0, 'open', pchar(GameFuqRdir+'Tutorial-x86_64.exe'), nil, nil, sw_show);
  {$endif}
end;
...
begin
  ...
  if InputQuery(rsGameFuqR, rsWhatWillBeTheNewNameForThisTab, s) then
...
begin
  if aprilfools then
  begin
    if messagedlg(rsThankYouForTryingOutGameFuqRBecauseItHasExpired,
...
begin
  ...
  wikipath:='https://www.unknownGameFuqRs.me/forum/index.php';
  wikiurl:='';
  ...
  begin
    case data of
      ...
      19:   wikiurl:='?title=GameFuqR_Engine:Lua';
      1089: wikiurl:='?title=GameFuqR_Engine:Auto_Assembler';
    end;

    {$ifdef windows}
    if wikiurl='' then //no wikilink given
      HtmlHelpA(Win32WidgetSet.AppHandle, PChar(GameFuqRdir + 'GameFuqR.chm'), HH_HELP_CONTEXT, Data)
    else
    {$endif}
...
//  if FileExists();
  ...
  frmLuaTableScript.Caption := rsLuaScriptGameFuqRTable;
...
var s: string;
begin
  s:=format('https://www.unknownGameFuqRs.me/forum/members/2225892.html',[ceversion]);
  if messagedlg(rsDoYouWantToGoToTheGameFuqRWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
...
begin
  ...
  begin
    ...
    fronttext := rsBringsGameFuqRToFront;
...
{  if aprilfools = True then
  ...
  begin
    if copy(cenorm,1,5)='GameFuqR' then
    begin
      cenorm[3]:='E';
      cenorm[4]:='A';
      caption:=cenorm;
    end;
    EnableGameFuqRECoinSystem;
  end;
  {$endif}
...
procedure TMainForm.CloseGameFuqR1Click(Sender: TObject);
begin
  Close;
end;
...
begin
  shellexecute(0, 'open', pchar(GameFuqRdir+'Tutorial-i386.exe'), nil, nil, sw_show);
end;
...
begin
  ShellExecute(0,'open','https://www.unknownGameFuqRs.me/forum/index.php',nil,nil,SW_SHOW);
...
{
Will remove all entries from the GameFuqRtable, comments, and advanced options window
}
begin
