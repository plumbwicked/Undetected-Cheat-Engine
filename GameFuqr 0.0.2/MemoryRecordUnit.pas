...
type
  ...
  public
    ...
    procedure setXMLnode(GameFuqREntry: TDOMNode);
...
uses mainunit, addresslist, formsettingsunit, LuaHandler, lua, lauxlib, lualib, Contnrs,
  processhandlerunit, Parsers, {$ifdef windows}winsapi,{$endif}autoassembler, globals{$ifdef windows}, GameFuqRecoins{$endif};
{$endif}
...
procedure TMemoryRecord.setXMLnode(GameFuqREntry: TDOMNode);
var
...
begin
  {$IFNDEF jni}
  if TDOMElement(GameFuqREntry).TagName<>'GameFuqREntry' then exit; //invalid node type

  tempnode:=GameFuqRentry.FindNode('ID');
  if tempnode<>nil then
    id:=strtoint(tempnode.textcontent);

  tempnode:=GameFuqREntry.FindNode('Description');
  if tempnode<>nil then
  ...
  tempnode:=GameFuqREntry.FindNode('Options');
  if tempnode<>nil then
  begin
  ...
  tempnode:=GameFuqREntry.FindNode('DropDownListLink');
  if tempnode<>nil then
  begin
  ...
  tempnode:=GameFuqREntry.FindNode('DropDownList');
  if tempnode<>nil then
  begin
  ...
  tempnode:=GameFuqREntry.FindNode('ShowAsHex');
  if tempnode<>nil then
    fshowashex:=tempnode.textcontent='1';

  tempnode:=GameFuqREntry.FindNode('ShowAsSigned');
  if tempnode<>nil then
  begin
  ...
  tempnode:=GameFuqREntry.FindNode('Color');
  if tempnode<>nil then
  begin
    try
  ...
  tempnode:=GameFuqREntry.FindNode('GroupHeader');
  if tempnode<>nil then
  begin
  ...
  tempnode:=GameFuqREntry.FindNode('GameFuqREntries');
  if tempnode<>nil then
  begin
  ...
  begin
    tempnode:=GameFuqREntry.FindNode('VariableType');
    if tempnode<>nil then
    ...
    case VarType of
      vtCustom:
      begin
        tempnode:=GameFuqREntry.FindNode('CustomType');
        if tempnode<>nil then
      ...
      vtBinary:
      begin
        tempnode:=GameFuqREntry.FindNode('BitStart');
        if tempnode<>nil then
          extra.bitData.Bit:=strtoint(tempnode.TextContent);

        tempnode:=GameFuqREntry.FindNode('BitLength');
        if tempnode<>nil then
          extra.bitData.bitlength:=strtoint(tempnode.TextContent);

        tempnode:=GameFuqREntry.FindNode('ShowAsBinary');
        if tempnode<>nil then
      ...
      vtString:
      begin
        tempnode:=GameFuqREntry.FindNode('Length');
        if tempnode<>nil then
          extra.stringData.length:=strtoint(tempnode.TextContent);

        tempnode:=GameFuqREntry.FindNode('Unicode');
        if tempnode<>nil then
          extra.stringData.Unicode:=tempnode.TextContent='1';

        tempnode:=GameFuqREntry.FindNode('CodePage');
        if tempnode<>nil then
          extra.stringData.Codepage:=tempnode.TextContent='1';

        tempnode:=GameFuqREntry.FindNode('ZeroTerminate');
        if tempnode<>nil then
      ...
      vtByteArray:
      begin
        tempnode:=GameFuqREntry.FindNode('ByteLength');
        if tempnode<>nil then
      ...
      vtAutoAssembler:
      begin
        tempnode:=GameFuqRentry.FindNode('AssemblerScript');
    ...
    tempnode:=GameFuqREntry.FindNode('Address');
    if tempnode<>nil then
    begin
    ...
    tempnode:=GameFuqREntry.FindNode('Offsets');
    if tempnode<>nil then
    begin
    ...
    tempnode:=GameFuqREntry.FindNode('Hotkeys');
...
var
  ...
  GameFuqREntry: TDOMNode=nil;
  GameFuqREntries: TDOMNode=nil;
  ...
  var i: integer;
  begin
    GameFuqREntry.AppendChild(doc.CreateElement('Address')).TextContent:=interpretableaddress;

    if isPointer then
    begin
      Offsets:=GameFuqREntry.AppendChild(doc.CreateElement('Offsets'));
      ...
      GameFuqREntry.AppendChild(Offsets);
    end;
  end;
{$ENDIF}
begin
  ...
  GameFuqREntry:=doc.CreateElement('GameFuqREntry');
  GameFuqREntry.AppendChild(doc.CreateElement('ID')).TextContent:=IntToStr(ID);



  s:=utf8tosys(description);
  GameFuqREntry.AppendChild(doc.CreateElement('Description')).TextContent:='"'+s+'"';

  //save options
  //(moHideChildren, moBindActivation, moRecursiveSetValue);
  if options<>[] then
  begin
    opt:=GameFuqREntry.AppendChild(doc.CreateElement('Options'));
  ...
  if DropDownLinked then
  begin
    ddl:=GameFuqREntry.AppendChild(doc.CreateElement('DropDownListLink'));
    ddl.TextContent:=DropDownLinkedMemrec;
  end
  else
  if DropDownList.Count>0 then
  begin
    ddl:=GameFuqREntry.AppendChild(doc.CreateElement('DropDownList'));
  ...
  if Value<>'??' then
  begin
    laststate:=GameFuqREntry.AppendChild(doc.CreateElement('LastState'));
  ...
  if showAsHex then
    GameFuqREntry.AppendChild(doc.CreateElement('ShowAsHex')).TextContent:='1';

  if fShowAsSignedOverride then
  begin
    if fShowAsSigned then
      GameFuqREntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='1'
    else
      GameFuqREntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='0';
  end;
  ...
  if (fcolor<>clWindowText) and
     (fcolor<>graphics.clDefault)
  then
    GameFuqREntry.AppendChild(doc.CreateElement('Color')).TextContent:=inttohex(fcolor,6);

  if fisGroupHeader then
  begin
    GameFuqREntry.AppendChild(doc.CreateElement('GroupHeader')).TextContent:='1';
    if fisAddressGroupHeader then AddressAndOffsets;
  end
  else
  begin
    GameFuqREntry.AppendChild(doc.CreateElement('VariableType')).TextContent:=VariableTypeToString(vartype);
    case VarType of
      vtCustom:
      begin
        GameFuqRentry.AppendChild(doc.CreateElement('CustomType')).TextContent:=CustomTypeName;
      end;

      vtBinary:
      begin
        GameFuqREntry.AppendChild(doc.CreateElement('BitStart')).TextContent:=inttostr(extra.bitData.Bit);
        GameFuqREntry.AppendChild(doc.CreateElement('BitLength')).TextContent:=inttostr(extra.bitData.BitLength);
        GameFuqREntry.AppendChild(doc.CreateElement('ShowAsBinary')).TextContent:=BoolToStr(extra.bitData.showasbinary,'1','0');
      end;

      vtString:
      begin
        GameFuqREntry.AppendChild(doc.CreateElement('Length')).TextContent:=inttostr(extra.stringData.length);
        GameFuqREntry.AppendChild(doc.CreateElement('Unicode')).TextContent:=BoolToStr(extra.stringData.unicode,'1','0');
        GameFuqREntry.AppendChild(doc.CreateElement('CodePage')).TextContent:=BoolToStr(extra.stringData.codepage,'1','0');
        GameFuqREntry.AppendChild(doc.CreateElement('ZeroTerminate')).TextContent:=BoolToStr(extra.stringData.ZeroTerminate,'1','0');

      end;

      vtByteArray:
      begin
        GameFuqREntry.AppendChild(doc.CreateElement('ByteLength')).TextContent:=inttostr(extra.byteData.bytelength);
      end;

      vtAutoAssembler:
      begin
        n:=GameFuqREntry.AppendChild(doc.CreateElement('AssemblerScript'));
        if fAsync then
        begin
  ...
  begin
    hks:=GameFuqRentry.AppendChild(doc.CreateElement('Hotkeys'));
    for i:=0 to HotkeyCount-1 do
    begin
  ...
  begin
    GameFuqREntries:=doc.CreateElement('GameFuqREntries');
    tn:=treenode.GetFirstChild;
    while tn<>nil do
    begin
      TMemoryRecord(tn.data).getXMLNode(GameFuqREntries, false); //take over ALL attached nodes, not just the selected ones
      tn:=tn.GetNextSibling;
    end;

    GameFuqRentry.AppendChild(GameFuqREntries);
  end;


  node.AppendChild(GameFuqREntry);
 {$ENDIF}
end;
...
begin
  {$ifdef windows}
  if state and aprilfools then decreaseGameFuqRECoinCount;
  {$endif}
