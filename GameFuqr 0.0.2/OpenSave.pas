...
resourcestring
  ...
  rsThisIsNotAValidGameFuqRTable = 'This is not a valid GameFuqR table';
  ...
  rsAskIfStupid = 'Generating a trainer with the current state of the GameFuqR '
    +'table will likely result in a completely useless trainer that does '
    +'nothing. Are you sure?';
  rsOSThereIsANewerVersionifGameFuqROutEtc = 'There is a newer version of Game FuqR out. It''s recommended to use that version instead';
  rsOSThisGameFuqRTableIsCorrupt = 'This GameFuqR table is corrupt';
...
begin
  ...
  try
    ...
    GameFuqRTable:=doc.FindNode('GameFuqRTable');
    if GameFuqRTable<>nil then
    begin

      signed:={$ifdef windows}isProperlySigned(TDOMElement(GameFuqRtable), signedstring, imagepos, image){$else}false{$endif};

      try
        tempnode:=GameFuqRTable.Attributes.GetNamedItem('GameFuqRTableVersion');
      except
        tempnode:=nil;
      end;
      ...
      begin
        try
          ...
          if (version>CurrentTableVersion) then
            showmessage(rsOSThereIsANewerVersionifGameFuqROutEtc);

          lastLoadedTableVersion:=version;
        except
          showmessage(rsOSThisGameFuqRTableIsCorrupt);
        end;
      end;


      Files:=GameFuqRTable.FindNode('Files');
      Forms:=GameFuqRTable.FindNode('Forms');
      Entries:=GameFuqRTable.FindNode('GameFuqREntries');
      Codes:=GameFuqRTable.FindNode('GameFuqRCodes');
      Symbols:=GameFuqRTable.FindNode('UserdefinedSymbols');
      Structures:=GameFuqRTable.FindNode('Structures');
      Comments:=GameFuqRTable.FindNode('Comments');
      LuaScript:=GameFuqRTable.FindNode('LuaScript');
      DComments:=GameFuqRTable.FindNode('DisassemblerComments');
    end;
...
begin
  ...
  try
    ...
    begin
      memfile.WriteBuffer(pchar('GameFuqR')^,11);
      temp:=2; //version
...
begin
  ...
  try
    ...
    begin
      ...
      if X='GameFuqR' then
      begin
         doc:=ConvertGameFuqRTableToXML(filename)
      end
      else
      begin
        //protected
    ...
    try
      ...
      on e: exception do
        raise exception.Create(rsThisIsNotAValidGameFuqRTable + ' ('+e.message+')');
    end;
...
var
  ...
  a: TDOMAttr;
begin
  GameFuqRTable:=TDOMElement(doc.AppendChild(TDOMNode(doc.CreateElement('GameFuqRTable'))));
  TDOMElement(GameFuqRTable).SetAttribute('GameFuqRTableVersion',IntToStr(CurrentTableVersion));

  if mainform.LuaForms.count>0 then
  begin
    Forms:=GameFuqRTable.AppendChild(doc.CreateElement('Forms'));
  ...
  begin
    Files:=GameFuqRTable.AppendChild(doc.CreateElement('Files'));
    for i:=0 to mainform.Luafiles.count-1 do
      mainform.LuaFiles[i].savetoxml(files);
  end;

  entries:=GameFuqRTable.AppendChild(doc.CreateElement('GameFuqREntries'));

  mainform.addresslist.saveTableXMLToNode(entries);

  if advancedoptions.count>0 then
  begin
    CodeRecords:=GameFuqRTable.AppendChild(doc.CreateElement('GameFuqRCodes'));
...
//        CodeRecord.Attributes.SetNamedItem(doc.CreateAttribute('GroupHeader')).TextContent:='1';
  ...
  try
    symbols:=GameFuqRTable.AppendChild(doc.CreateElement('UserdefinedSymbols'));
  ...
  begin
    Structures:=GameFuqRTable.AppendChild(doc.CreateElement('Structures'));
  ...
  if comments.memo1.Lines.Count>0 then
  begin
    comment:=GameFuqRTable.AppendChild(doc.CreateElement('Comments'));
  ...
  begin
    luascript:=GameFuqRTable.AppendChild(doc.CreateElement('LuaScript'));
  ...
  begin
    dcomments:=GameFuqRTable.AppendChild(doc.CreateElement('DisassemblerComments'));
...
begin
  ...
  begin
    ...
    f.Position:=5;  //skip "GameFuqR" header
  ...
  begin
    //no GameFuqR header, so old bad storage method
