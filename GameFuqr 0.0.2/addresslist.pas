...
type
  ...
  private
   ...
   // procedure TreeviewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    function GameFuqRTableNodeHasOnlyAutoAssemblerScripts(GameFuqRTable: TDOMNode): boolean; //helperfunction
    procedure GameFuqRTableNodeCheckForRelativeAddress(GameFuqRTable: TDOMNode; var hasRelative, allRelative: boolean); //helperprocedure
  ...
  public
    ...
    procedure refresh;
    procedure SelectAll;
    procedure saveTableXMLToNode(GameFuqREntries: TDOMNode; selectedOnly: boolean=false);
    procedure loadTableXMLFromNode(GameFuqREntries: TDOMNode);
...
procedure TAddresslist.saveTableXMLToNode(GameFuqREntries: TDOMNode; selectedOnly: boolean=false);
var tn: TTreenode;
p: TTreenode;
m: TMemoryRecord;
begin
  ...
  while tn<>nil do
  begin
    TMemoryRecord(tn.data).getXMLNode(GameFuqREntries,selectedonly);
...
procedure TAddresslist.loadTableXMLFromNode(GameFuqREntries: TDOMNode);
var currentEntry: TDOMNode;
memrec: TMemoryRecord;
i: integer;
begin

  currentEntry:=GameFuqREntries.FirstChild;
  while currententry<>nil do
  begin
    if tdomelement(currententry).TagName='GameFuqREntry' then
    begin
...
var
  doc: TXMLDocument;
  GameFuqRtable,GameFuqREntries: TDOMNode;
  i: integer;
  ms: TMemoryStream;
begin
  result:='';
  ...
  GameFuqRtable:=doc.CreateElement('GameFuqRTable');
  doc.AppendChild(GameFuqRtable);

  GameFuqREntries:=doc.CreateElement('GameFuqREntries');
  GameFuqRtable.AppendChild(GameFuqREntries);

  try
    saveTableXMLToNode(GameFuqREntries, selectedOnly);
    WriteXMLFile(doc,ms);
    ms.WriteByte(0);
...
function TAddresslist.GameFuqRTableNodeHasOnlyAutoAssemblerScripts(GameFuqRTable: TDOMNode): boolean;
{
private
checks if the given xml document contains GameFuqRentries that aren't aa scripts
}
var GameFuqREntries, currentEntry: TDOMNode;
  vtnode: TDOMNode;
begin
  result:=true;
  //go through the list untill one is found that has the custom type
  GameFuqREntries:=GameFuqRTable.FindNode('GameFuqREntries');
  if GameFuqRentries<>nil then
  begin
    currentEntry:=GameFuqREntries.FirstChild;
    while currententry<>nil do
    begin
...
procedure __GameFuqRTableNodeCheckForRelativeAddress(GameFuqRTable: TDOMNode; var hasRelative, allRelative: boolean);
var GameFuqREntries, currentEntry: TDOMNode;
  addrnode: TDOMNode;
  s: string;
begin
  ...
  begin
    ...
    begin
      ...
      if currententry.findnode('GameFuqREntries')<>nil then
        __GameFuqRTableNodeCheckForRelativeAddress(currentEntry, hasRelative, allRelative);
...
procedure TAddresslist.GameFuqRTableNodeCheckForRelativeAddress(GameFuqRTable: TDOMNode; var hasRelative, allRelative: boolean);
{
private
checks if the given xml document contains GameFuqRentries without relative Address
}
begin
  hasRelative:=false;
  allRelative:=true;
  __GameFuqRTableNodeCheckForRelativeAddress(GameFuqRTable, hasRelative, allRelative);
end;
          ...
          begin
            ...
            try
              ...
              begin
                ...
                if not GameFuqRTableNodeHasOnlyAutoAssemblerScripts(GameFuqRTable) then
            ...
            GameFuqRTableNodeCheckForRelativeAddress(GameFuqRTable, hasRelative, allRelative);
