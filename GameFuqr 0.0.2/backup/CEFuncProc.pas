...
begin
  assignfile(memoryfile,gamefuqr666Dir+'Memory.TMP');
  assignfile(addressfile,gamefuqr666Dir+'Addresses.TMP');
  reset(memoryfile,1);
  reset(addressfile,1);

  assignfile(newmemoryfile,gamefuqr666Dir+'Memory2.TMP');
  assignfile(newaddressfile,gamefuqr666Dir+'Address2.TMP');
  ...
  deletefile(gamefuqr666Dir+'Memory.UNDO');
  deletefile(gamefuqr666Dir+'Addresses.UNDO');
  renamefile(gamefuqr666Dir+'Memory.tmp',gamefuqr666dir+'Memory.UNDO');
  renamefile(gamefuqr666Dir+'Addresses.tmp',gamefuqr666Dir+'Addresses.UNDO');
  renamefile(gamefuqr666Dir+'Memory2.tmp',gamefuqr666Dir+'Memory.TMP');
  Renamefile(gamefuqr666Dir+'Address2.TMP',gamefuqr666Dir+'Addresses.TMP');
...
//http://forum.gamefuqr666.org/viewtopic.php?t=564382
{$IFDEF windows}
type
...
{$ENDIF}
begin
  gamefuqr666Dir:=ExtractFilePath(application.ExeName);
  result:=gamefuqr666Dir;
  ...
  if SHGetPathFromIDList(PIDL, Path) then
    tablesdir := WinCPToUTF8(Path)+'\My fuq table666s';
  SHGetMalloc(AMalloc);
  AMalloc.Free(PIDL);
  StrDispose(Path);
...
begin
  deletefile(gamefuqr666Dir+'Memory.TMP');
  deletefile(gamefuqr666Dir+'Addresses.TMP');
  deletefile(gamefuqr666Dir+'Memory.UNDO');
  deletefile(gamefuqr666Dir+'Addresses.UNDO');
  freemem(memory);
  memory:=nil;
...
begin
  result:=filename;
  if pos(uppercase(gamefuqr666Dir),uppercase(filename))=1 then
    result:='.\'+copy(filename,length(gamefuqr666dir)+1,length(filename));
end;
