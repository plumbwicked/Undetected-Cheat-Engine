...
{
Modified for Game FuqR use because filenames differ between 32 and 64-bit (also different po file, but just pick the one with the most data...)
...
var
  ...
  begin
    ...
    begin
      ...
      Result := GameFuqRdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'GameFuqR'+LCEXT;
      if FileExists(Result) then exit;

      Result := GameFuqRdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'GameFuqR-x86_64'+LCEXT;
      if FileExists(Result) then exit;

      Result := GameFuqRdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'GameFuqR-i386'+LCEXT;
      if FileExists(Result) then exit;
...
begin
  ...
  if (lang = '') and (FileExists(GameFuqRdir+ 'languages' + DirectorySeparator+'language.ini')) then
  begin
    try
      lini:=TIniFile.Create(GameFuqRdir+'languages' + DirectorySeparator+'language.ini');
      try
