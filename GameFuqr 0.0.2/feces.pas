...
procedure signTable(GameFuqRtable: TDOMElement);
procedure signTableFile(f: string);
function isProperlySigned(GameFuqRtable: TDOMElement; out specialstring: string; out imagepos: integer; out image: tpicture): boolean;
...
resourcestring
  ...
  rsSelectYourGameFuqRSignatureFile = 'Select your Game FuqR signature '
    +'file';
  rsGameFuqRSignatureFiles = 'Game FuqR signature files';
  ...
  rsFailedToLoadGameFuqRPublicKey = 'Failed to load Game FuqR public key';
...
var
  GameFuqRpublictablekey: BCRYPT_KEY_HANDLE=0;
...
procedure getXmlfileWithoutSignature(GameFuqRtable: TDOMElement; output: tstream);
var
  signature: TDOMNode;
begin
  signature:=GameFuqRTable.FindNode('Signature');

  if signature<>nil then
  begin
    GameFuqRTable.DetachChild(signature);
    signature.free;
  end;

  if GameFuqRtable<>nil then
  begin
    //showmessage(GameFuqRtable.TextContent);
    WriteXML(GameFuqRtable,output);
//    output.WriteAnsiString(GameFuqRtable.TextContent);
  end;
end;
...
var
  ...
  GameFuqRtablecontents: TMemorystream=nil;
...
begin
  ...
  try
    ...
    if GameFuqRpublictablekey=0 then
    begin
      s:=BCryptImportKeyPair(hAlgoritm, 0, BCRYPT_ECCPUBLIC_BLOB, GameFuqRpublictablekey, @publictablekey[0], 140, 0);
      if not succeeded(s) then raise exception.create(
        rsFailedToLoadGameFuqRPublicKey);
    end;
    ...
    s:=BCryptVerifySignature(GameFuqRpublictablekey,nil,hashbuffer,hashlength,sig, signaturesize,0);
    ...
    GameFuqRtablecontents:=TMemoryStream.create;
    getXmlfileWithoutSignature(GameFuqRtable, GameFuqRtablecontents);

    s:=BCryptCreateHash(hashAlgoritm, hHash, bHashObject, objectlength, nil, 0, 0);
    if not Succeeded(s) then raise exception.create(rsFailedCreatingHash2);

    s:=BCryptHashData(hHash, GameFuqRtablecontents.Memory, GameFuqRtablecontents.size, 0);
    ...
    if GameFuqRtablecontents<>nil then
      freeandnil(GameFuqRtablecontents);
  ...
  //check the signatureless version of the GameFuqR table with this public key
end;
...
begin
  ReadXMLFile(d, f);
  e:=TDOMElement(d.FindNode('GameFuqRTable'));
  signtable(e);
  WriteXMLFile(d,f);
end;
...
begin
  ...
  doc:=GameFuqRtable.OwnerDocument;
  ...
  begin
    ...
    try
      od.Title:=rsSelectYourGameFuqRSignatureFile;
      od.Filter:=rsGameFuqRSignatureFiles+'|*.CESIG';
  ...
  try
    ...
    try
      ...
      //showmessage(pchar(GameFuqRtablecontents.Memory));
      ...
      signature:=GameFuqRTable.AppendChild(doc.CreateElement('Signature'));
