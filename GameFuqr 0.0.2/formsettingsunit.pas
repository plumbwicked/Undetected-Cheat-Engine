...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in Game FuqR finding NO memory! Are you stupid?';
  ...
  rsPopupHideGameFuqR = 'Popup/Hide Game FuqR';
  ...
  rsRestartCE = 'It is recommended to restart Game FuqR for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.{$ifdef windows}WriteBinaryData{$else}WriteString{$endif}('Show Game FuqR Hotkey',{$ifndef windows}bintohexs({$endif}frameHotkeyConfig.newhotkeys[1][0],10){$ifndef windows}){$endif};
        ...
        begin
          if reg.OpenKey('\Software\Game FuqR\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\Game FuqR\Tools');
      if Reg.OpenKey('\Software\Game FuqR\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\Game FuqR\Plugins'+cpu);
      if Reg.OpenKey('\Software\Game FuqR\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    ...
    try
      ini:=TIniFile.Create(GameFuqRdir+'languages' + DirectorySeparator+'language.ini');
      try
...
begin
  ...
  FindAllDirectories(f,GameFuqRDir+'\languages',false);
  for i:=0 to f.Count-1 do
  begin
    n:=f[i];
    if not (fileexists(n+'\GameFuqR.po') or fileexists(n+'\GameFuqR-x86_64.po') or fileexists(n+'\GameFuqR-i386.po')) then
      continue;
