...
begin
  ...
  begin
    ...
    MainForm.frmLuaTableScript.assemblescreen.Lines.Insert(2, '  messageDialog(''Please install Game FuqR ''..RequiredCEVersion, mtError, mbOK)');
  ...
  try
    ...
    if CopyFile(GameFuqRdir+basefile+'.dat', filename) then
    begin
      ...
      begin
        ...
        begin
          ...
          decompressor.LoadFromFile(GameFuqRdir+'standalonephase2.dat');
          ...
          addfile(GameFuqRdir+'defines.lua');

          if rb32.checked then
          begin
            addfile(GameFuqRdir+'GameFuqR-i386.exe');

            addfile(GameFuqRdir+'lua53-32.dll');
            addfile(GameFuqRdir+'win32\dbghelp.dll','win32');

            if cbSpeedhack.checked then
              addfile(GameFuqRdir+'speedhack-i386.dll');

            if cbvehdebug.checked then
              addfile(GameFuqRdir+'vehdebug-i386.dll');

            if cbKernelDebug.checked then
            begin
              addfile(GameFuqRdir+'dbk32.sys');
              addfile(GameFuqRdir+'dbk64.sys');
              addfile(GameFuqRdir+'GameFuqR-i386.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(GameFuqRdir+'libmikmod32.dll');


            if cbCCode.checked then
              addfile(GameFuqRdir+'tcc32-32.dll');
          end
          else
          begin
            addfile(GameFuqRdir+'GameFuqR-x86_64.exe');
            addfile(GameFuqRdir+'lua53-64.dll');

            if cbSpeedhack.checked then
              addfile(GameFuqRdir+'speedhack-x86_64.dll');

            if cbvehdebug.checked then
              addfile(GameFuqRdir+'vehdebug-x86_64.dll');

            if cbKernelDebug.checked then
            begin
              addfile(GameFuqRdir+'dbk64.sys');
              addfile(GameFuqRdir+'GameFuqR-x86_64.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(GameFuqRdir+'libmikmod64.dll');

            if cbCCode.checked then
              addfile(GameFuqRdir+'tcc64-64.dll');
          end;

          if cbIncludes.checked then
            addfolder(GameFuqRdir+'include','include');


          if cbDotNet.checked then
          begin
            addfile(GameFuqRdir+'DotNetDataCollector32.exe');
            addfile(GameFuqRdir+'DotNetDataCollector64.exe');
          end;

          if cbD3DHook.checked then
          begin
            addfile(GameFuqRdir+'overlay.fx');
            if rb32.checked then
            begin
              addfile(GameFuqRdir+'d3dhook.dll');
              addfile(GameFuqRdir+'ced3d9hook.dll');
              addfile(GameFuqRdir+'ced3d10hook.dll');
              addfile(GameFuqRdir+'ced3d11hook.dll');
            end
            else
            begin
              addfile(GameFuqRdir+'d3dhook64.dll');
              addfile(GameFuqRdir+'ced3d9hook64.dll');
              addfile(GameFuqRdir+'ced3d10hook64.dll');
              addfile(GameFuqRdir+'ced3d11hook64.dll');
            end;
          end;
      ...
      showMessage(Format(rsTrainerFileMissing, [GameFuqRdir+basefile, filename]));
...
begin
  ...
  d:=ExtractRelativepath(GameFuqRdir, d);
...
begin
  ...
  OpenDialog1.InitialDir:=GameFuqRDir;
  SelectDirectoryDialog1.InitialDir:=GameFuqRDir;
