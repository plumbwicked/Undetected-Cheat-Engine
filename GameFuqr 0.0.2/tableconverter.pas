...
function ConvertGameFuqRTableToXML(filename: string): TXMLDocument;
...
begin
  ...
  try
    ...
    GameFuqRTable:=doc.AppendChild(doc.CreateElement('GameFuqRTable'));
    ...
    begin
      ...
      GameFuqREntry.AppendChild(doc.CreateElement('Description')).TextContent:='Group '+inttostr(i);

      groups[i].node:=GameFuqREntry;
      groups[i].appendnode:=GameFuqREntry.AppendChild(doc.CreateElement('GameFuqREntries'));
      groups[i].used:=false;
    end;
    ...
    begin
      ...
      begin
        ...
        t:=GameFuqREntry.AppendChild(doc.CreateElement('Description'));
       ...
       // showmessage(x+' = '+t.textcontent);
        ...
        Address:=GameFuqREntry.AppendChild(doc.CreateElement('Address'));
        ...
        GameFuqREntry.AppendChild(doc.CreateElement('VariableType')).TextContent:=VariableTypeToString(vtype);

        ctfile.ReadBuffer(tempbyte,1);
        if tempbyte=0 then
          GameFuqREntry.AppendChild(doc.CreateElement('Unicode')).TextContent:='0'
        else
          GameFuqREntry.AppendChild(doc.CreateElement('Unicode')).TextContent:='1';

        ctfile.ReadBuffer(tempbyte,1); //bit, could be used for bitstart or stringlength, glad that is separate now
        GameFuqREntry.AppendChild(doc.CreateElement('Length')).TextContent:=inttostr(tempbyte);
        GameFuqREntry.AppendChild(doc.CreateElement('ByteLength')).TextContent:=inttostr(tempbyte);


        GameFuqREntry.AppendChild(doc.CreateElement('BitStart')).TextContent:=inttostr(tempbyte);

        ctfile.ReadBuffer(tempdword,4); //bitlength
        GameFuqREntry.AppendChild(doc.CreateElement('BitLength')).TextContent:=inttostr(tempdword);
        ...
        begin
          ...
          groups[tempbyte].AppendNode.AppendChild(GameFuqREntry); //append this entry to the group
        end
        else
          entries.AppendChild(GameFuqREntry); //append this entry to the main table

        ctfile.ReadBuffer(tempbyte,1); //showashex
        if tempbyte=0 then
          GameFuqREntry.AppendChild(doc.CreateElement('ShowAsHexadecimal')).TextContent:='0'
        else
          GameFuqREntry.AppendChild(doc.CreateElement('ShowAsHexadecimal')).TextContent:='1';
        ...
        ctfile.readbuffer(x^,j);
        x[j]:=#0;
        GameFuqREntry.AppendChild(doc.CreateElement('AssemblerScript')).TextContent:=x;
        freememandnil(x);
      end;
