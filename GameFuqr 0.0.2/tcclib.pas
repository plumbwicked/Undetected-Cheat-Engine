...
begin
  ...
  {$ifdef cpu32}
  module:=LoadLibrary({$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'tcc32-32.dll'); //generates 32-bit code
  {$else}
  if target=x86_64 then
    module:=loadlibrary({$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'tcc64-64.dll')
  else
    module:=loadlibrary({$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'tcc64-32.dll'); //generates 32-bit code
  {$endif}
  {$else}
  module:=loadlibrary({$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'libtcc.dylib');
  if module=0 then
  begin
...
var
  ...
  i: integer;
begin
  add_include_path(s,{$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'include');
  {$ifdef windows}
  add_include_path(s,{$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'include\winapi');
  {$endif}
  add_include_path(s,{$ifdef standalonetest}'D:\git\GameFuqR-engine\Game FuqR\bin\'+{$endif}'include\sys');
