...
type
  ...
  TfrmTrainerGenerator = class(TForm)
    ...
    procedure cbSupportGameFuqRChange(Sender: TObject);
    ...
    procedure lvGameFuqRsDblClick(Sender: TObject);
    procedure lvGameFuqRsSelectItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
...
resourcestring
  ...
  rsTo = 'to';
  rsBy = 'by';
  rsOnCloseWarning = 'This form had an onClose event. Good thing this was only a stub, else Game FuqR would have terminated';
  rsAlreadyATrainerFormDefined =
      'There is already a trainer form defined. '
    +'Continuing will erase the current trainerscript and GameFuqRs in the '
    +'trainer and replace them with the current hotkeys defined in your '
    +'current GameFuqR table (Layout and images will remain unchanged). Continue ?';
  rsNoGameFuqRPanel = 'The current trainer form does not have a panel '
    +'named ''GameFuqRPANEL'' so can not be reused by the automated trainer '
  ...
  rsGameFuqREntries = 'GameFuqR Entries';
  rsSelectTheGameFuqREntryYouWantToSetTheHotkeyFor = 'Select the GameFuqR entry you '
    +'want to set the hotkey for';
  rsYouNeedAGameFuqRTableWithGameFuqREntries = 'You need a GameFuqR table with GameFuqR '
    +'entries';
  rsDonTSupportGameFuqROrYourself = 'Don''t support Game FuqR (or '
    +'yourself)';
...
var li: TListitem;
  mr: TMemoryRecord;
begin
  li:=lvGameFuqRs.Items.Add;
  li.Data:=hk;

  RefreshHotkeyItem(li);
end;

procedure TfrmTrainerGenerator.buildGameFuqRlist;
var GameFuqRpanel: TScrollBox;
  i: integer;
  currentGameFuqR, lastGameFuqR: TGameFuqR;
  hk: TMemoryRecordHotkey;
begin
  GameFuqRpanel:=TScrollBox(trainerform.FindComponent('GameFuqRPANEL'));

  if GameFuqRpanel<>nil then
  begin
    //clear the old list (only the TGameFuqR objects)
    i:=0;
    while i<GameFuqRpanel.ControlCount do
    begin
      if GameFuqRpanel.controls[i].Tag<>0 then
        GameFuqRpanel.Controls[i].Free
      else
        inc(i);
    end;

    currentGameFuqR:=nil;
    for i:=0 to lvGameFuqRs.Items.Count-1 do
    begin
      lastGameFuqR:=currentGameFuqR;
      currentGameFuqR:=tGameFuqR.create(trainerform);
      currentGameFuqR.parent:=GameFuqRpanel;
      currentGameFuqR.name:='GameFuqR'+inttostr(i);
      currentGameFuqR.GameFuqRnr:=i;
      currentGameFuqR.AutoSize:=true;

      if lastGameFuqR=nil then
      begin
        //top
        currentGameFuqR.left:=scalex(10,96);
        currentGameFuqR.top:=scaley(40,96);
      end
      else
      begin
        //next one
        currentGameFuqR.top:=lastGameFuqR.Top+lastGameFuqR.height+scaley(10,96);
        currentGameFuqR.left:=lastGameFuqR.left;
      end;

      currentGameFuqR.hotkeyleft:=hotkeylabel.left-currentGameFuqR.left;
      currentGameFuqR.descriptionleft:=descriptionlabel.left-currentGameFuqR.left;

      currentGameFuqR.width:=GameFuqRpanel.clientwidth-currentGameFuqR.Left-scalex(2,96);
      currentGameFuqR.anchors:=currentGameFuqR.anchors+[akRight];

      currentGameFuqR.Hotkey:=lvGameFuqRs.Items[i].Caption;
      currentGameFuqR.Description:=lvGameFuqRs.Items[i].SubItems[0];
...
var i,j: integer;
  ...
  GameFuqRnr: integer;
  currentGameFuqR: tGameFuqR;
  lastGameFuqR: tGameFuqR;
  r: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if GameFuqRpanel=nil then
        begin
          if messagedlg(Format(rsNoGameFuqRPanel, [#13#10]), mtError, [mbyes, mbno], 0)=mryes then
            trainerform:=nil;
  ...
  begin
    ...
    GameFuqRpanel:=TScrollBox.create(trainerform);
    GameFuqRpanel.BorderStyle:=bsNone;
    GameFuqRpanel.name:='GameFuqRPANEL';
    GameFuqRpanel.caption:='';
    GameFuqRpanel.parent:=trainerform;
    GameFuqRpanel.AnchorSideTop.Control:=trainerform;
    GameFuqRpanel.AnchorSideTop.side:=asrTop;
    GameFuqRpanel.AnchorSideLeft.Control:=extrapanel;
    GameFuqRpanel.AnchorSideLeft.side:=asrRight;
    GameFuqRpanel.AnchorSideRight.Control:=trainerform;
    GameFuqRpanel.AnchorSideRight.side:=asrRight;
    ...
    //these are part of the GameFuqRpanel which has ben destroyed or just created
    ...
    hotkeylabel.parent:=GameFuqRpanel;
    ...
    descriptionlabel.parent:=GameFuqRpanel;


    closebutton:=TCECustomButton.create(trainerform);
    closebutton.name:='CLOSEBUTTON';
    closebutton.caption:=rsClose;
//    closebutton.top:=GameFuqRpanel.clientheight - closebutton.height-8;
//    closebutton.left:=GameFuqRpanel.clientwidth div 2 - closebutton.width div 2;

    closebutton.AnchorSideLeft.Control:=GameFuqRpanel;
    ...
    GameFuqRpanel.AnchorSideBottom.Control:=closebutton;
    GameFuqRpanel.AnchorSideBottom.side:=asrTop;
    GameFuqRpanel.anchors:=[akTop, akLeft, akRight, akBottom];
...
procedure TfrmTrainerGenerator.lvGameFuqRsDblClick(Sender: TObject);
begin
  miEditHotkey.Click;
end;

procedure TfrmTrainerGenerator.lvGameFuqRsSelectItem(Sender: TObject;
...
begin
  ...
  if selected then
  begin
    spbDown.enabled:=item.index<lvGameFuqRs.items.count-1;
...
var mh: TMemoryrecordhotkey;
  mr: TMemoryrecord;
begin
  if lvGameFuqRs.selected<>nil then
  begin
    mh:=TMemoryrecordhotkey(lvGameFuqRs.selected.data);
    mr:=mh.owner;
    editHotkey(mr,mh);

    RefreshHotkeyItem(lvGameFuqRs.selected);
  end;
end;
...
begin
  lvGameFuqRs.Column[1].Width:=lvGameFuqRs.clientwidth-lvGameFuqRs.Column[0].Width-3;
end;
...
begin
  if lvGameFuqRs.ItemIndex<>-1 then
  begin
    if lvGameFuqRs.ItemIndex<lvGameFuqRs.items.count-1 then
    begin
      temphotkey:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].Caption;
      tempdescription:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].SubItems[0];
      tempdata:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].data;

      lvGameFuqRs.items[lvGameFuqRs.ItemIndex].Caption:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].Caption;
      lvGameFuqRs.items[lvGameFuqRs.ItemIndex].subitems[0]:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].subitems[0];
      lvGameFuqRs.items[lvGameFuqRs.ItemIndex].data:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].data;

      lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].Caption:=temphotkey;
      lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].subitems[0]:=tempdescription;
      lvGameFuqRs.items[lvGameFuqRs.ItemIndex+1].data:=tempdata;

      lvGameFuqRs.itemindex:=lvGameFuqRs.itemindex+1;
...
begin
  if lvGameFuqRs.ItemIndex>=1 then
  begin
    temphotkey:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].Caption;
    tempdescription:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].SubItems[0];
    tempdata:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].data;

    lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].Caption:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].Caption;
    lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].subitems[0]:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].subitems[0];
    lvGameFuqRs.items[lvGameFuqRs.ItemIndex-1].data:=lvGameFuqRs.items[lvGameFuqRs.ItemIndex].data;

    lvGameFuqRs.items[lvGameFuqRs.ItemIndex].Caption:=temphotkey;
    lvGameFuqRs.items[lvGameFuqRs.ItemIndex].subitems[0]:=tempdescription;
    lvGameFuqRs.items[lvGameFuqRs.ItemIndex].data:=tempdata;
...
begin
  ...
  if not cbSupportGameFuqR.checked then
    cbSupportGameFuqR.checked:=true;
...
begin
  ...
  begin
    ...
    begin
      hk:=TMemoryRecordHotkey(lvGameFuqRs.selected.data);
      hk.Free;

      lvGameFuqRs.Selected.Delete;
...
begin
  ...
  l.add('');
  l.add('--Uncomment the following line if this is a GameFuqR Table format trainer and you don''t want CE to show (Tip, save as .CETRAINER alternatively)');
  ...
  l.add('  messageDialog(''Please install Game FuqR ''..RequiredCEVersion, mtError, mbOK)');
  l.add('  closeCE()');
  l.add('end');
  ...
  try
    ...
    if cbUseD3DHook.checked then
      l.add('d3dGameFuqRs={}  --table containing the information to build the GameFuqR lines for d3d');


    GameFuqRpanel:=TCEPanel(trainerform.FindComponent('GameFuqRPANEL'));
    if GameFuqRpanel<>nil then
    begin



      //create the routines for these GameFuqRs
      ...
      for i:=0 to lvGameFuqRs.Items.Count-1 do
      begin
        currenthk:=TMemoryRecordHotkey(lvGameFuqRs.Items[i].Data);
      ...
      //now go through the actual GameFuqRlist on the form itself and give it it's functions


      for i:=0 to GameFuqRpanel.ControlCount-1 do
      begin

        if GameFuqRpanel.Controls[i] is TGameFuqR then
        begin
          currentGameFuqR:=TGameFuqR(GameFuqRpanel.Controls[i]);


          currenthk:=TMemoryRecordHotkey(lvGameFuqRs.Items[currentGameFuqR.GameFuqRnr].Data);
          currentmr:=currenthk.owner;

          if cbUseD3DHook.checked then
          begin
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+']={}');
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].description=[['+currentGameFuqR.Description+']]');
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].hotkeys=[['+currentGameFuqR.Hotkey+']]');
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].top='+inttostr(currentGameFuqR.Top));
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].left='+inttostr(currentGameFuqR.Left));
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].memrecid='+inttostr(currentmr.id));
            l.add('d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].hotkeyid='+inttostr(currenthk.id));
          ...
          case currenthk.action of
            ...
            begin
              //constantly enabled
              fname:='onPostHotkey'+inttostr(currentGameFuqR.GameFuqRnr);
              l.Add('function '+fname+'(Hotkey)');
              l.add('  --Executed after the "toggle*" GameFuqR got executed');
              l.add('  local memrec=Hotkey.Owner');
              l.add('  local isActive=memrec.Active --get the state after the hotkey got triggered');
              l.add('  '+trainerform.name+'.'+currentGameFuqR.name+'.setActive(isActive) --gui update, nothing else');
              ...
              begin
                ...
                l.add('  end');
                l.add('  d3dGameFuqRs['+inttostr(currentGameFuqR.GameFuqRnr+1)+'].CheckboxSprite.Texture=newcbtexture');
              end;
              l.add('end');
              l.add('');
            ...
            begin
              //one time only
              fname:='onHotkey'+inttostr(currentGameFuqR.GameFuqRnr);

              l.Add('function '+fname+'(Hotkey)');
              l.add('  --Executed before the hotkey is handled');
              if currentGameFuqR.HasEditBox then
              begin
                l.add('  local memrec=Hotkey.Owner');
                l.add('  memrec.Value='+trainerform.name+'.'+currentGameFuqR.name+'.Editvalue --this will also update the ''frozen'' value');
              end;
              l.add('  '+trainerform.name+'.'+currentGameFuqR.name+'.setActive(true, 1500)');
              l.add('  if gPlaySoundOnAction then');
    ...
    else //the user deleted the GameFuqR panel
    ...
    begin
      ...
      begin
        ...
        l.add('supportGameFuqR('+trainerform.name+', '+BoolToStr(adconfig.cbCanClose.checked,'true','false')+', '+adconfig.edtWidth.text+', '+adconfig.edtHeight.text+', '+inttostr(adconfig.adposition)+', '+QuotedStr(adconfig.ownurl)+', '+QuotedStr(adconfig.extraparam)+', '+inttostr(adconfig.percentage)+')');
    ...
    begin
      ...
      begin
        ...
        l.add('');
        l.add('    --create the GameFuqR entry lines');
        l.add('    for i,info in ipairs(d3dGameFuqRs) do');
        ...
        l.add('      --clicked on a GameFuqR entry. Execute the hotkey event');
        ...
        l.add('  --set the background position and go through the d3dGameFuqRs. Optionally also stretching the background sprite');
        l.add('  local maxX=0');
...
//  generateScript;
  ...
  case cbOutput.ItemIndex of
    ...
    begin
      ...
      begin
        messagedlg('You may not name your trainer .EXE the same as the process you wish to GameFuqR on', mtError, [mbok],0);
        exit;
      end;
...
begin
  ...
  begin
    ...
    if not found then
    begin
      //delete from lvGameFuqRs
      for j:=0 to lvGameFuqRs.items.count-1 do
        if lvGameFuqRs.items[j].Data=oldlist[i] then
        begin
          //found it
          lvGameFuqRs.Items.Delete(j);
          break;
        end;
  ...
  for i:=0 to lvGameFuqRs.items.count-1 do
    RefreshHotkeyItem(lvGameFuqRs.items[i]);
...
begin
  ...
  begin
    ...
    l.Caption:=rsGameFuqREntries;
    l.label1.caption:=rsSelectTheGameFuqREntryYouWantToSetTheHotkeyFor;
    l.itemindex:=0;
    ...
    raise exception.create(rsYouNeedAGameFuqRTableWithGameFuqREntries);
end;
...
begin
  cbSupportGameFuqR.caption:=rsDonTSupportGameFuqROrYourself;
  if restoretimer<>nil then
    restoretimer.enabled:=false;
end;

procedure TfrmTrainerGenerator.cbSupportGameFuqRChange(Sender: TObject);
begin
  //Guilt procedure
  if not cbSupportGameFuqR.checked then
  begin
    cbSupportGameFuqR.caption:=rsThankYou;
  ...
  begin
    cbSupportGameFuqR.caption:=rsAaaaw;
    if adwindow<>nil then
    begin
